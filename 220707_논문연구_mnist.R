...
#################################################################################
#dimension reduction using sparse dr
################################################################################
...
# y label : 2, 5, 9만 추출
#######################################################
mnist[,785] = as.numeric(mnist[,785])-1
mnist = rbind(mnist[mnist[,785]==2,],
              mnist[mnist[,785]==5,],mnist[mnist[,785]==9,])

set.seed(2022)
train = sample(1:20261, 15000)
mnist_train = mnist[train,]
mnist_test = mnist[-train,]

#######################################################
# split train/test
x.tra <- mnist_train[,1:784] ; dim(x.tra)
x.tes <- mnist_test[,1:784] ; dim(x.tes)
y.tra <- mnist_train[,785]
y.tes <- mnist_test[,785]

#############################################################################################################################
############################################################################################################################
############################################################################################################################

# sdr 사용해서 classification 성능 
...
ssdr.result.tra <- ssdr.lambda(x.tra, y.tra, method="dr", d=10, nslices=10, lambda1=rep(0.01,10), lambda2=0.01, max.iter=100, eps.conv=1e-7)
beta.tra = beta.order(ssdr.result.tra$beta)
x.tra.sdr =  as.matrix(x.tra)%*%beta.tra
...
#ssdr.result.tes <- ssdr.lambda(x.tes, y.tes, method="dr", d=10, nslices=10, lambda1=rep(0.01,10), lambda2=0.01, max.iter=100, eps.conv=1e-7)
#beta.tes = beta.order(ssdr.result.tes$beta)
#x.tes.sdr =  as.matrix(x.tes)%*%beta.tes

############################################################
# 패키지 RSNNS의 mlp 함수로 다중 퍼셉트론 모델 학습
############################################################
library(RSNNS)
model <- mlp(x.tra.sdr, decodeClassLabels(y.tra), size=c(10,10,10),learnFuncParams=c(0.5), maxit=100)
# hidden layer가 3개이고, 각각 노드가 10인 mlp 모델

end_time <- Sys.time()
sdr_model_time <- end_time - start_time

summary(model)
plotIterativeError(model)

#########################################################
# 학습된 mlp 모델로 test셋 예측하기
###########################################################
predictions <- predict(model, x.tes.sdr)


##########################################################
# confusion matrix 작성
##########################################################
tt <- table(y.tes, encodeClassLabels(predictions))
...
# dr 사용해서 classificiation 성능
start_time <- Sys.time()

dr.result.tra <- comp.sdr(x.tra, y.tra, method="dr", d=10, nslices=10)
x.tra.dr =  as.matrix(x.tra)%*%dr.result.tra$beta.sdr

#dr.result.tes <- comp.sdr(x.tes, y.tes, method="dr", d=10, nslices=10)
#x.tes.dr =  as.matrix(x.tes)%*%dr.result.tes$beta.sdr

############################################################
# 패키지 RSNNS의 mlp 함수로 다중 퍼셉트론 모델 학습
############################################################
library(RSNNS)
model <- mlp(x.tra.dr, decodeClassLabels(y.tra), size=c(10,10,10),learnFuncParams=c(0.1), maxit=100)
# hidden layer가 3개이고, 각각 노드가 10인 mlp 모델

end_time <- Sys.time()
dr_model_time <- end_time - start_time

plotIterativeError(model)
#########################################################
# 학습된 mlp 모델로 test셋 예측하기
###########################################################
predictions <- predict(model, x.tes.dr)
...
# sdr 사용하지 않고 원래 데이터셋에서의 classification 성능
start_time <- Sys.time()
############################################################
# 패키지 RSNNS의 mlp 함수로 다중 퍼셉트론 모델 학습
############################################################
library(RSNNS)
model <- mlp(x.tra, decodeClassLabels(y.tra), size=c(300,100,10),learnFuncParams=c(0.1), maxit=100)
# hidden layer가 3개이고, 각각 노드가 4인 mlp 모델

end_time <- Sys.time()
origin_model_time <- end_time - start_time
...
# 학습된 mlp 모델로 test셋 예측하기
###########################################################
predictions <- predict(model, x.tes)
