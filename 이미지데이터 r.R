...
library(RCurl)
library(dplyr)
imag <- readPNG("C:/Users/82105/Desktop/논문/논문연구/data/elpv-dataset-master/elpv-dataset-master/images/cell0001.png")
imag
dim(imag)
image(imag)
img_mat=t(apply(imag, 2, rev))
image(img_mat)
...
########################################################################################33
#mono에 대한 이미지만 사용
img_inf_csv = read.csv("C:/Users/82105/Desktop/논문/논문연구/data/elpv-dataset-master/elpv-dataset-master/labels.csv", header=T)
img_mono_csv = img_inf_csv %>% filter(img_inf_csv$type == "mono")

img_url = "C:/Users/82105/Desktop/논문/논문연구/data/elpv-dataset-master/elpv-dataset-master/"
img.matrix <- matrix(0,nrow(img_mono_csv),90000)
#img.list <- list()
for(i in 1:nrow(img_mono_csv)){
  img <- as.vector(readPNG(paste0(img_url, img_mono_csv$image_name[i])))
  img.matrix[i,] <- img
}

img.matrix <- cbind(img.matrix, img_mono_csv$defect_prob)  
dim(img.matrix)
...
# split train/test
set.seed(2022)
idx <- sample(1:nrow(img.matrix), size = nrow(img.matrix)*0.7, replace=FALSE)
x.tra <- img.matrix[idx,1:90000] ; dim(x.tra)
x.tes <- img.matrix[-idx,1:90000] ; dim(x.tes)
y.tra <- img.matrix[idx,90001]
y.tes <- img.matrix[-idx,90001]
...
ssdr.result1 <- ssdr.lambda(x.tra, y.tra, method="dr", d=2, nslices=4, lambda1=c(1,1), lambda2=0.1, max.iter=100, eps.conv=1e-7)
...
# results <- list()
#for(j in 1:length(img.list)){
#  pic<- img.list[[j]]/max(img.list[[j]])
#  results[[j]] <- pic*225
#}
